...
unsigned long KeepAliveInterval = 600;
unsigned long ConnectionTimeOutInterval = 300;
bool retain = false;
int Qos = 1;
...
int a, b, c, d, e = 0; //for voltage Measure * indicator
...
float Vrms380 = 400.00;     // Take form measurement tools
...
void MQTThandling() { // handling renew connection MQTT Client "ESP32" to Broker
  if (!mqttClient.connect(broker, port)) {
    ...
    if(mqttClient.connectError()){
      ...
      Serial.print("exTime2:");Serial.println(deltaTask2); // one execution handling procedure +- 1000000 us
    }
  }
}
...
void blink_led3() { // Indicator for low MQTT connection
  for(d=0; d<=4; d++){
    digitalWrite(13, HIGH); delay(100);
    digitalWrite(13, LOW); delay(100);
  }
...
void Task1code( void * pvParameters ){
  ...
  for(;;){
//    unsigned long startTask1 = micros();
    voltageMeasure();
    #ifdef DEBUG
    ...
    #endif
    delay(1);
//    unsigned long endTask1 = micros();
//    unsigned long deltaTask1 = endTask1 - startTask1;
//    Serial.print("exTime1:");Serial.println(deltaTask1);
  } 
}
